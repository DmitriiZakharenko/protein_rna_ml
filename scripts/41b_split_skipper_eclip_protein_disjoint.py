#!/usr/bin/env python3
"""
41b_split_skipper_eclip_protein_disjoint.py
-------------------------------------
Protein-disjoint train/val/test splits **within Skipper eCLIP only** (protein-disjoint).
...
Typical workflow (replicate RPIembeddor-style protein-disjoint comparison with V2 CNN):
...
-----
  ...
  python scripts/41b_split_skipper_eclip_protein_disjoint.py \\
    ...
    --out_dir data/benchmarks/skipper_eclip/protein_disjoint \\
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Protein-disjoint eCLIP train/val/test splits"
    )
    ap.add_argument(
        "--pairs_tsv",
    ...
    ap.add_argument(
        "--out_dir",
        default="data/benchmarks/skipper_eclip/protein_disjoint",
    )
    ap.add_argument(
        "--summary_json",
        default="results/skipper_eclip/protein_disjoint_split_summary.json",
    )
    ...
    summary = {
        "script": "41b_split_skipper_eclip_protein_disjoint.py",
        "description": "Protein-disjoint split within Skipper eCLIP (RPIembeddor-style protein-disjoint style)",
    ...
    print("\n=== protein-disjoint eCLIP protein split ===")
